    ...
    try {
        ...
        if (APP_STATE.currentUser.uid === targetedProductItemMatch.ownerUid) {
            operationalActionControlsLayoutStringHTML = `
                <button class="btn-gray" style="flex: 1;" onclick="closeActiveModalDirectly('product-detail-modal'); switchSettingsSection('my-products'); navigateToPage('my-account');">⚙️ Manage Products</button>
            `;
        } else {
            operationalActionControlsLayoutStringHTML = `
                <button class="btn-blue" style="flex: 1;" onclick="closeActiveModalDirectly('product-detail-modal'); initialDirectMessageCommunicationPipelineSetup('${targetedProductItemMatch.ownerUid}')">💬 Message Seller</button>
            `;
        }
        ...
        detailOverlayBodyNode.innerHTML = `
            ...
            <div class="modal-expanded-content-split-grid margin-top-md" style="display:grid; grid-template-columns: 1fr 1fr; gap:24px;">
                ...
                <div class="expanded-right-details-column" style="display:flex; flex-direction:column; gap:12px;">
                    ...
                    <div class="modal-expanded-actions-footer-row btn-group" style="margin-top:auto; padding-top:16px; border-top:1px solid #f0f0f0; display: flex; gap: 10px;">
                        ${operationalActionControlsLayoutStringHTML}
                        <button class="btn-green" style="flex: 1;" onclick="shareProductModalDetails('${targetedProductItemMatch.pid}')">🔗 Share</button>
                    </div>
                </div>
            </div>
        `;
...
function appendProductRowToContainer(containerElement, item, productId) {
    ...
    itemRowRowStripContainerElementNode.innerHTML = `
        ...
        <div style="display:flex; gap:8px;">
            ...
            <button class="btn-danger" style="padding:4px 10px; font-size:0.75rem;" onclick="executeDeletePlatformInventoryItemListingPostRecord('${productId}')">Delete</button>
        </div>
    `;
...
// ============================================================================
// FIREBASE FIRESTORE SYNC HELPERS (FETCH & SAVE)
// ============================================================================

// Fetch leaderboard ownership, expiration times, and slot mappings from Firebase
async function fetchLeaderboardStateFromFirebaseStorage() {
    if (window.FortMartFirebase && window.FortMartFirebase.db) {
        try {
            const { db, doc, getDoc } = window.FortMartFirebase;
            const targetDocRef = doc(db, "system_metadata", "leaderboardConfig");
            const docSnap = await getDoc(targetDocRef);

            if (docSnap.exists()) {
                const data = docSnap.data();
                if (data.pinnedLeaderboard) {
                    SYSTEM_DATABASE.pinnedLeaderboard = data.pinnedLeaderboard;
                }
                if (data.slotMetadata) {
                    SYSTEM_DATABASE.slotMetadata = data.slotMetadata;
                }
                console.log("--> Leaderboard configuration successfully fetched from Firebase Firestore.");
            } else {
                console.log("--> No leaderboard configuration document found in Firebase.");
            }
        } catch (error) {
            console.error("Firebase Storage Fetch Error:", error);
            showTopRightToast("Warning: Failed to load leaderboard data from Firebase.", "error");
        }
    } else {
        console.warn("Firebase SDK context not found on window.FortMartFirebase.");
    }
}

// Persist slot mapping and expiration details to Firebase
async function saveLeaderboardStateToFirebaseStorage() {
    if (window.FortMartFirebase && window.FortMartFirebase.db) {
        try {
            const { db, doc, setDoc } = window.FortMartFirebase;
            const targetDocRef = doc(db, "system_metadata", "leaderboardConfig");
            await setDoc(targetDocRef, {
                pinnedLeaderboard: SYSTEM_DATABASE.pinnedLeaderboard,
                slotMetadata: SYSTEM_DATABASE.slotMetadata,
                updatedAt: Date.now()
            }, { merge: true });
            console.log("--> Leaderboard configuration successfully persisted to Firebase Firestore Storage.");
        } catch (error) {
            console.error("Firebase Storage Persist Error:", error);
            showTopRightToast("Warning: Failed to sync changes with Firebase database.", "error");
        }
    } else {
        console.warn("Firebase SDK context not found on window.FortMartFirebase.");
    }
...
async function renderLeaderboardInterfaceSectionInSettings(parentElementContainer) {
    ...
    // Fetch latest ownership, slot mappings, and expiry dates from Cloud Firestore
    await fetchLeaderboardStateFromFirebaseStorage();
    ...
    for (let idx = 0; idx < 20; idx++) {
        ...
        if (isActuallyOwned && !isExpired) {
            ...
            if (isExpired && !isInGracePeriod && slotMeta.previousOwnerUid) {
                ...
                // Persist expired state cleanup back to Firestore
                saveLeaderboardStateToFirebaseStorage();
            }
        }
...
async function processPaystackPaymentSuccess(slotIndex) {
    const now = Date.now();
    ...
    // Persist new ownership and expiration to Firebase Firestore
    await saveLeaderboardStateToFirebaseStorage();
...
async function saveSlotAllocationSettingsMapping(slotIndex) {
    const dropdown = document.getElementById("slot-product-mapping-dropdown");
    if (!dropdown) return;
    ...
    // Persist modifications directly into Firebase Cloud Firestore Storage
    await saveLeaderboardStateToFirebaseStorage();

    const modal = document.getElementById("manage-slot-allocation-modal");
    if (modal) modal.remove();
...
function cancelActiveSlotSubscription(slotIndexPositionId) {
    ...
    displayConfirmationModalOverlayAction("Are you sure you want to cancel this slot positioning subscription setup?", async () => {
        ...
        // Persist cancellation to Firebase Storage
        await saveLeaderboardStateToFirebaseStorage();
...
function launchDetailedUserProfileContextOverlaySummaryModal(userIdTokenKeyParameterValue, pushHistory = true) {
        ...
        administrativeControlsInlineHTML = `
            <div style="margin-top:12px; margin-bottom:12px; padding:14px; background:#f7fafc; border:1px solid #cbd5e0; border-radius:8px; display:flex; flex-direction:column; gap:10px;">
                ...
                <div class="btn-group" style="margin-top:4px;">
                    <button class="btn-blue" style="padding:6px 12px; font-size:0.8rem;" onclick="executeInlineAdminSave('${uid}')">Apply Policy Changes</button>
    ...
    standardModalBodyElementNode.innerHTML = `
        <div class="modal-expanded-header-row" style="display:flex; justify-space-between; align-items:center; border-bottom:1px solid var(--fort-gray-border); padding-bottom:14px;">
...
/**
 * Executes administrative updates and saves changes directly to Firestore and local state
 */
async function executeInlineAdminSave(targetUid) {
    const identifierInput = document.getElementById("adm-user-identifier-text")?.value.trim();
    const passwordInput = document.getElementById("adm-user-security-password")?.value.trim();
    const verificationCodeInput = document.getElementById("UserAccountAuthenticationVerificationCode")?.value.trim();
    const accountTypeInput = document.getElementById("adm-change-account-type")?.value;
    const statusTag = document.getElementById("lbl-inspector-active-status-tag");
    const statusInput = statusTag ? statusTag.getAttribute("data-pending-status-value") : "unverified";

    const updatePayload = {
        identifierText: identifierInput,
        secretKey: passwordInput,
        password: passwordInput,
        UserAccountAuthenticationVerificationCode: verificationCodeInput,
        verificationCode: verificationCodeInput,
        accountType: accountTypeInput,
        type: accountTypeInput,
        status: statusInput,
        verificationStatus: statusInput
    };
    ...
    try {
        ...
        const localUser = SYSTEM_DATABASE.users.find(u => u.uid === targetUid || u.id === targetUid);
        if (localUser) {
            Object.assign(localUser, updatePayload);
        }

        if (typeof showTopRightToast === "function") {
            showTopRightToast("Admin policy changes updated successfully!", "success");
        } else {
            alert("Admin policy changes updated successfully!");
        }

        // Refresh overlay with updated memory values
        launchDetailedUserProfileContextOverlaySummaryModal(targetUid, false);

    } catch (err) {
        console.error("Failed to commit inline admin changes to Firestore:", err);
        alert("Failed to save changes down to the server database. Please check connection.");
    }
}
